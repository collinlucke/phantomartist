...
import React, { forwardRef, useRef, useLayoutEffect, useState, useEffect, useCallback, createElement } from 'react';
...
const InputField = ({ label, name, type = 'text', value, onChange, placeholder, required = false, error, disabled = false, readonly = false, id, labelPosition = 'above', size = 'medium', autoResize = false, className, onKeyDown, onDark = false, testId, helperText, autoFocus = false, autoComplete, ariaLabel, ariaDescribedBy, ariaInvalid, ariaRequired, role, tabIndex, maxLength, minLength, pattern, min, max, step }) => {
    ...
    const labelElement = label ? (jsxs("label", { css: [
            ...
            className?.label
        ], htmlFor: inputId, children: [label, required && (jsx("span", { css: localStyles$a({ labelPosition, size, autoResize, onDark }).required, "aria-label": "required", children: "*" }))] })) : null;
    ...
    const helperTextElement = helperText ? (jsx("div", { css: localStyles$a({ labelPosition, size, autoResize }).helperText, id: `${inputId}-helper`, children: helperText })) : null;
...
const Main = ({ children, className, isDark = false }) => {
    return (jsx("main", { className: "pa-main", css: [localStyles$9(isDark).main, className?.main], children: children }));
};
...
const Header = ({ children, className, dataTestId, ariaLabel = 'Site header', ariaLabelledBy, role = 'banner' }) => {
    ...
    return (jsx("header", { css: [localStyles$7.header, className?.header], className: "pa-header", "data-testid": dataTestId, role: role, "aria-label": ariaLabel, "aria-labelledby": ariaLabelledBy, children: jsx(Fragment, { children: children }) }));
};
const localStyles$7 = {
    header: {
        display: 'flex',
...
const Block = ({ children, className, dataTestId }) => {
    return (jsx("div", { css: [localStyles$6.block, className?.block], className: "pa-block", "data-testid": dataTestId, children: children }));
};
const localStyles$6 = {
    block: {
        width: '100%',
...
const List = ({ className, children, ariaLabel, ariaLabelledBy, ariaDescribedBy, role = 'list', dataTestId, ordered = false }) => {
    ...
    return (jsx(ListElement, { css: [localStyles$5.list, className], className: "pa-list", role: role, "aria-label": ariaLabel, "aria-labelledby": ariaLabelledBy, "aria-describedby": ariaDescribedBy, "data-testid": dataTestId, children: children }));
};
const localStyles$5 = {
    list: {
        fontWeight: '500',
...
const AccordionListItem = ({ title, children, isOpen = false, onToggle, disabled = false, className, ariaLabel, dataTestId, useAnimation = true }) => {
    ...
    return (jsxs("li", { css: [localStyles$3.accordionItem, className?.item], className: "pa-accordion-item", "data-testid": dataTestId, children: [jsx(Button, { className: {
                ...
                ], className: "pa-accordion-content-wrapper", children: jsx("div", { ref: contentRef, css: [localStyles$3.content, className?.content], className: "pa-accordion-content", id: `accordion-content-${dataTestId || 'default'}`, role: "region", "aria-labelledby": `accordion-header-${dataTestId || 'default'}`, children: children }) })] }));
};
const localStyles$3 = {
    accordionItem: {
        width: '100%',
...
const AccordionList = ({ items, allowMultiple = false, defaultOpenItems = [], onItemToggle, className, useAnimation = true, ariaLabel, dataTestId }) => {
    ...
    return (jsx("ul", { css: [localStyles$2.accordionList, className?.list], className: "pa-accordion-list", role: "list", "aria-label": ariaLabel || 'Accordion list', "data-testid": dataTestId, children: items.map(item => (jsx(AccordionListItem, { title: item.title, isOpen: openItems.has(item.id), onToggle: () => handleItemToggle(item.id), disabled: item.disabled, useAnimation: useAnimation, className: {
                item: className?.item,
...
export { AccordionList, AccordionListItem, Block, Button, ButtonGroup, Globals, Header, Image, InnerWidth, InputField, List, ListItem, Main, Modal, index as PAHooks, Search, baseColors, baseTheme, baseTypography, baseVibrantColors, createResponsiveStyle, hexToRgba, mediaQueries, screenSizes };
